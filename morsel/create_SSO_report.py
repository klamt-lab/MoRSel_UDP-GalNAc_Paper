...
data_dict = {'Model': [run_ID for run_ID in dict_of_param_ensembles.keys()], 
             ...
             'AIC': [np.round(720*np.log(param_ensemble.min()['obj']/720)+2*((len(param_ensemble.columns)-2)+1), 2) for param_ensemble in dict_of_param_ensembles.values()], # AIC = n*log(RSS/n)+2*k; k = num_estim_params+1; 720 exp. data points
             'AICc': [np.round(720*np.log(param_ensemble.min()['obj']/720)+((2*((len(param_ensemble.columns)-2)+1)*(((len(param_ensemble.columns)-2)+1)+1))/(720-((len(param_ensemble.columns)-2)+1)-1)), 2) for param_ensemble in dict_of_param_ensembles.values()], # AICc = n*log(RSS/n)+((2*k*(k+1))/(n-k-1)); k = num_estim_params+1; 720 exp. data points
             'BIC': [np.round(720*np.log(param_ensemble.min()['obj']/720)+np.log(720)*((len(param_ensemble.columns)-2)+1), 2) for param_ensemble in dict_of_param_ensembles.values()], # BIC = n*log(RSS/n)+log(n)*k; k = num_estim_params+1; 720 exp. data points
             'CIC1': [np.round(720*np.log(param_ensemble.min()['obj']/720)+(np.abs((720*np.log(param_ensemble.min()['obj']/720)))*0.01)*((len(param_ensemble.columns)-2)+1), 2) for param_ensemble in dict_of_param_ensembles.values()], # CIC1 = n*log(RSS/n)+(abs((n*log(RSS/n)))*0.01)*k; k = num_estim_params+1; 720 exp. data points
             'CIC2': [np.round(720*np.log(param_ensemble.min()['obj']/720)+200*((len(param_ensemble.columns)-2)+1), 2) for param_ensemble in dict_of_param_ensembles.values()], # CIC2 = n*log(RSS/n)+200*k; k = num_estim_params+1; 720 exp. data points
             'CIC3': [np.round(720*np.log(param_ensemble.min()['obj']/720)*1/((len(param_ensemble.columns)-2)+1), 2) for param_ensemble in dict_of_param_ensembles.values()], # CIC3 = (n*log(RSS/n))*1/k; k = num_estim_params+1; 720 exp. data points
