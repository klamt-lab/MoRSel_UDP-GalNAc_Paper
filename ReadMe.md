...
<summary>Overview of all figures and tables of the manuscript that are based on results generated by code in this repository</summary>

| Figure/Table      | Source                                                                                                                                                                                         |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Figure 4          | based on image files in [the toymodel subdirectory](morsel/toymodel/noise_std_0.2_num_candidates)                                                                                              |
| Table 3           | data taken from [IESv23b_SSO_report_numerical_traces.csv](morsel/IESv23b_SSO_report_numerical_traces.csv)                                                                                      |
...
Check the [data](/morsel/toymodel/noise_std_0.2_num_candidates_5/true_model_data_noise_std_0.2.txt) generated by the toy model used for the rediscovery test as another example.

### 3. Estimating the parameter values of a model variant
...
### Rediscovery Test (Toy Model)
The *MoRSel* method was evaluated by performing a so called "rediscovery test". Its aim is to test whether, given data generated from a hidden true model, *MoRSel* is able to rediscover the structure (and parameterization) of this true model. The rediscovery test was performed using the toy model as defined in the [*MoRSel* function library](/morsel/func_lib.py) and was implemented in [this Python script file](/morsel/toymodel/run_rediscovery_tests.py) with all results stored in a [subdirectory](/morsel/toymodel/noise_std_0.2_num_candidates_5).

### UDP-GalNAc Model
*MoRSel* was applied to refine an initial model of the UDP-GalNAc process in order to generate a modified model variant that better describes the data. The complete framework was repeated five times. The results of the first run are located directly in the [*MoRSel* directory](/morsel). The results of the other four repetitions are located in the directories: [rep1](/morsel/rep1), [rep2](/morsel/rep2), [rep3](/morsel/rep3), and [rep4](/morsel/rep4). A [report](/morsel/IESv23b_SSO_report.csv) that summarizes the results of all five runs was generated by running [this script](/morsel/create_SSO_report.py) (it also lists the results of additional test runs that were calculated to analyze the effect of only using a subset of MSC for all model ranking steps).
...
The results of both optimizations were analyzed and visualized by the following Python scripts: [cross_validation_check_constraints](/process_optimization/cross_validation_check_constraints.py), [create_boxplots_and_heatmaps](/process_optimization/create_boxplots_and_heatmaps.py) and [visualize_selected_predictions](/process_optimization/visualize_selected_predictions.py). The figures generated by the latter two were saved in the model-specific subdirectories of the optimization setups A and B.
